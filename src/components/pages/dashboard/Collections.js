...
const Collections = () => {
  ...
  useEffect(() => {
    dispatch(readAllCollectionsAsync());
  ...
  return (
    <>
          ...
          <div className="max-w-7xl px-5 sm:px-10  mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <table className="shadow mt-2 w-full divide-y divide-cool-gray-200 table-fixed">
              ...
              <tbody className="bg-white divide-y divide-cool-gray-200">
                {(collection.collections || []).map((collection, i) => (
                  <tr
                    key={i}
                    className={`${
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right whitespace-no-wrap text-sm leading-5 text-cool-gray-500">
                      {collection.Platform && collection.Platform.name}
                    </td>
      ...
      <FormModal
        ...
        Form={CollectionForm}
        title={model ? "Update collection" : "Add new collection"}
        model={model}
      />
      <DeleteConfirmationModal
        ...
        deleteAsync={() => {
          dispatch(deleteCollectionAsync({ collection: model })).then(action => {
            if (!action.error) {
